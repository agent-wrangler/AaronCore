...
def _task_plan_continuation_markers() -> tuple[str, ...]:
    return (
        "continue",
        "resume",
        "follow up",
...
def _looks_like_task_plan_continuation(query: str) -> bool:
    ...
    return any(normalized.startswith(marker) for marker in _task_plan_continuation_markers())
...
def _looks_like_direct_task_resume_command(query: str) -> bool:
    ...
    for marker in _task_plan_continuation_markers():
...
def _find_matching_task_plan_task(user_input: str = "", preferred_fs_target: str = ""):
    ...
    if _looks_like_task_plan_continuation(raw):
        if _looks_like_direct_task_resume_command(raw):
            return latest
        if last_ref and (last_ref in raw or raw in last_ref or _goal_overlap(raw, last_ref) >= 0.2):
            return latest
        return None
...
def task_to_working_state(task: dict | None) -> dict | None:
    ...
    fs_target = _extract_task_fs_target(task) or get_structured_fs_target_for_task_plan(snapshot)
