...
from storage.paths import RUNTIME_STORE_DIR
from storage.paths import SELF_REPAIR_REPORTS_FILE as _SELF_REPAIR_REPORTS_FILE


ROOT = Path(__file__).resolve().parent.parent
STATE_DIR = RUNTIME_STORE_DIR
REPORTS_FILE = _SELF_REPAIR_REPORTS_FILE
FEEDBACK_RULES_FILE = _FEEDBACK_RULES_FILE
MAX_REPORTS = 80
...
def _routing_issue_files() -> list[dict]:
    return _dedupe_items(
        [
            {
                "path": "decision/routing/route_resolver.py",
                "reason": "Check active route fallback logic first.",
            },
            {
                "path": "routes/chat.py",
                "reason": "Check the main chat chain entry and post-LLM integration.",
            },
            {
                "path": "agent_final.py",
                "reason": "Check bootstrap wiring and shared exports that still affect the route path.",
            },
        ]
    )


def _routing_issue_tests() -> list[dict]:
    return _dedupe_items(
        [
            {
                "path": "tests/test_feedback_classifier.py",
                "reason": "Verify routing-style feedback still lands in the routing bucket.",
            },
            {
                "path": "tests/test_trace_payload.py",
                "reason": "Verify repair progress payloads still surface correctly for feedback flows.",
            },
        ]
    )
...
def _story_length_files() -> list[dict]:
    return _dedupe_items(
        [
            {"path": "skills/builtin/story.py", "reason": "Check story body length and continuation behavior."},
            {"path": "agent_final.py", "reason": "Check story packaging and continuation wrapper output."},
        ]
    )
...
def _story_length_tests() -> list[dict]:
    return _dedupe_items(
        [
            {"path": "tests/test_story_skill.py", "reason": "Verify story length and direct skill output."},
            {"path": "tests/test_story_routing.py", "reason": "Verify story follow-up turns still continue the same thread."},
        ]
    )
...
def _generic_chat_files() -> list[dict]:
    return _dedupe_items(
        [
            {"path": "routes/chat.py", "reason": "Check the main chat chain first."},
            {"path": "agent_final.py", "reason": "Check bootstrap and fallback reply wiring."},
            {"path": "brain/__init__.py", "reason": "Check local fallback reply shaping if the main model falls back."},
        ]
    )
...
def _generic_chat_tests() -> list[dict]:
    return _dedupe_items(
        [
            {"path": "tests/test_feedback_classifier.py", "reason": "Verify feedback vs. fresh request separation still works."},
            {"path": "tests/test_trace_payload.py", "reason": "Verify plain chat and correction payloads remain distinct."},
        ]
    )
...
def _ability_files() -> list[dict]:
    return _dedupe_items(
        [
            {"path": "routes/chat.py", "reason": "Check capability answers on the main chain."},
            {"path": "agent_final.py", "reason": "Check capability and fallback wrapping."},
            {"path": "brain/__init__.py", "reason": "Check local fallback capability wording."},
        ]
    )
...
def _ability_tests() -> list[dict]:
    return _dedupe_items(
        [
            {"path": "tests/test_feedback_classifier.py", "reason": "Verify capability-style corrections still map to chat/style issues."},
            {"path": "tests/test_trace_payload.py", "reason": "Verify plain chat does not get repair progress by mistake."},
        ]
    )
...
def _build_issue_profile(rule_item: dict) -> dict:
    ...
    if problem == "wrong_skill_selected" or fix == "adjust_skill_routing_for_scene":
        return {
            ...
            "diagnosis": "This looks like the route layer sent the request into the wrong execution branch. Check upstream routing and main-chain entry before patching skill internals.",
            "candidate_files": _routing_issue_files(),
            "suggested_tests": _routing_issue_tests(),
        }

    if problem == "length_too_short" or fix == "story_should_expand_when_user_requests_more" or scene == "story":
        return {
            "issue_key": "story_length",
            "label": "Story too short",
            "diagnosis": "This looks like a story generation or continuation issue. Check story output length first, then inspect the wrapper that returns story results to the main chain.",
    ...
    if problem == "fallback_too_generic" or fix == "ability_queries_should_answer_capabilities_directly":
        return {
            ...
            "label": "Capability answer too generic",
            "diagnosis": "This looks like a capability-answer shaping problem rather than a missing ability. Check the direct-answer path before adding more routing logic.",
    ...
    return {
        ...
        "diagnosis": "This feedback is broad. Start from the main reply layer and active chat chain, then expand into skill internals only if the smaller fix is insufficient.",
...
def _build_patch_prompt(report: dict) -> str:
    ...
    return (
        f"Issue: {report.get('label', 'unknown')}\n"
        f"Summary: {report.get('summary', '')}\n"
        f"Diagnosis: {report.get('diagnosis', '')}\n"
        f"Candidate files: {', '.join(file_paths) or 'none'}\n"
        f"Suggested tests: {', '.join(test_paths) or 'none'}\n"
        "Repair principle: change the smallest upstream layer first, keep edits minimal, and prefer validation before broader expansion."
    ).strip()
...
def _is_safe_auto_apply_path(path_str: str) -> bool:
    ...
    if not (rel.startswith("skills/builtin/") or rel.startswith("tools/agent/")):
        return False
...
def _build_patch_generation_prompt(report: dict, contexts: list[dict]) -> str:
    ...
    for item in contexts:
        blocks.append(
            "\n".join(
                [
                    f"File: {item['path']}",
                    f"Why inspect it: {item['reason'] or 'unspecified'}",
    ...
    return (
        "You are a conservative code repair planner. Generate the smallest possible patch plan for this self-repair report.\n\n"
        ...
        '      "old": "exact original text copied from the provided file content",\n'
        ...
        "}\n\n"
        "Rules:\n"
        "1. Only edit the provided candidate files.\n"
        "2. Do not create or delete files.\n"
        "3. Each old snippet must be an exact contiguous match from the file content.\n"
        "4. Keep edits minimal and prefer upstream fixes over downstream patches.\n"
        "5. Use at most 6 edits. If unsure, return edits: [].\n\n"
        "Candidate file contexts:\n\n"
        + "\n\n".join(blocks)
    ).strip()
...
def _build_report_summary(rule_item: dict, profile: dict) -> str:
    ...
    label = str((profile or {}).get("label") or "閫氱敤绾犲亸").strip()
    if question and feedback:
        return f"{label}: 用户刚才问“{question[:40]}”，随后反馈“{feedback[:36]}”，这轮先做一次最小修正提案。"
    if feedback:
...
def build_self_repair_report(rule_item: dict, config: dict | None = None) -> dict:
    ...
    report = {
        ...
        "proposed_actions": [
            ...
            "改完先跑建议测试，确认没把已有链路带坏，再决定要不要继续扩展。",
        ],
