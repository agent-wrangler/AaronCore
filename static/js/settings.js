...
function renderSettingsPage(isLight){
 ...
 html+='<button onclick="setLang(\'zh\')" style="padding:6px 14px;border-radius:8px;border:1px solid '+borderColor+';background:'+(_isZh?'linear-gradient(135deg,#6b7280,#8b5cf6)':cardBg)+';color:'+(_isZh?'#fff':textColor)+';font-size:13px;font-weight:600;cursor:pointer;">'+t('settings.lang.zh')+'</button>';
 html+='<button onclick="setLang(\'en\')" style="padding:6px 14px;border-radius:8px;border:1px solid '+borderColor+';background:'+(!_isZh?'linear-gradient(135deg,#6b7280,#8b5cf6)':cardBg)+';color:'+(!_isZh?'#fff':textColor)+';font-size:13px;font-weight:600;cursor:pointer;">'+t('settings.lang.en')+'</button>';
 html+='</div></div>';
 ...
 html+='<div style="background:'+cardBg+';padding:16px;border-radius:14px;border:1px solid '+borderColor+';"><div style="font-size:12px;color:'+subColor+';margin-bottom:8px;">'+t('settings.behavior')+'</div><div style="font-size:22px;font-weight:800;color:'+(l7s.l7_constraint_count?'#10b981':textColor)+';">'+(l7s.l7_constraint_count||0)+'<span style="font-size:12px;font-weight:400;color:'+subColor+';margin-left:4px;">'+t('settings.items')+'</span></div></div>';
...
function renderModelManageSection(isLight,textColor,subColor,cardBg,borderColor){
 ...
 if(!_settingsModels||!_settingsCatalog){
  ...
  for(var pk in catalog){
   ...
   var headerBg=isExpanded?(isLight?'rgba(99,102,241,0.04)':'rgba(99,102,241,0.06)'):('transparent');
   ...
   if(activeModelName){
    html+='<span style="padding:3px 10px;border-radius:5px;background:'+(isLight?'rgba(52,211,153,0.1)':'rgba(52,211,153,0.12)')+';color:#34d399;font-size:11px;font-weight:600;">'+escapeHtml(activeModelName)+'</span>';
   ...
   if(isExpanded){
    ...
    for(var j=0;j<configured.length;j++){
     ...
     var visionDot=cm.vision?'<span style="width:6px;height:6px;border-radius:50%;background:#60a5fa;display:inline-block;margin-left:4px;" title="'+t('settings.models.vision')+'"></span>':'';
     ...
     if(isCurrent){
      html+='<span style="padding:3px 10px;border-radius:5px;background:'+(isLight?'rgba(52,211,153,0.1)':'rgba(52,211,153,0.12)')+';color:#34d399;font-size:11px;font-weight:600;">'+t('settings.models.active')+'</span>';
     }else{
      html+='<button onclick="switchModel(\''+escapeHtml(cmid)+'\')" style="padding:3px 10px;border-radius:5px;border:1px solid '+(isLight?'rgba(99,102,241,0.25)':'rgba(99,102,241,0.3)')+';background:none;color:#6366f1;font-size:11px;cursor:pointer;font-weight:500;transition:all 0.15s;" onmouseenter="this.style.background=\'#6366f1\';this.style.color=\'#fff\'" onmouseleave="this.style.background=\'none\';this.style.color=\'#6366f1\'">'+t('settings.models.activate')+'</button>';
     }
    ...
    for(var m=0;m<catalogModels.length;m++){
     ...
     html+='<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:'+(isLight?'rgba(148,163,184,0.04)':'rgba(148,163,184,0.04)')+';border:1px dashed '+(isLight?'rgba(148,163,184,0.2)':'rgba(148,163,184,0.12)')+';border-radius:10px;">';
  ...
  if(uncategorized.length>0){
   ...
   if(uncExpanded){
    ...
    for(var u=0;u<uncategorized.length;u++){
     ...
     var uRowBg=uCurrent?(isLight?'rgba(59,130,246,0.06)':'rgba(59,130,246,0.1)'):(isLight?'#f8fafc':'rgba(28,28,30,0.6)');
     var uRowBorder=uCurrent?(isLight?'rgba(59,130,246,0.25)':'rgba(96,165,250,0.3)'):borderColor;
     ...
     if(uCurrent) html+='<span style="color:#34d399;font-size:14px;margin-right:6px;">\u2713</span>';
     ...
     if(uCurrent){
      ...
      html+='<button onclick="switchModel(\''+escapeHtml(umid)+'\')" style="padding:3px 10px;border-radius:5px;border:1px solid '+(isLight?'rgba(99,102,241,0.25)':'rgba(99,102,241,0.3)')+';background:none;color:#6366f1;font-size:11px;cursor:pointer;font-weight:500;">'+t('settings.models.activate')+'</button>';
     }
...
function openModelDialog(editId){
 ...
 var textColor=isLight?'#1c1c1e':'#e2e8f0';
 var subColor=isLight?'#64748b':'#94a3b8';
 var inputBg=isLight?'#f8fafc':'rgba(255,255,255,0.06)';
 ...
 html+='<input id="mdlVision" type="checkbox" '+checked+' style="width:16px;height:16px;accent-color:#6366f1;">';
 ...
 // 删除按钮（编辑模式 + 非当前模型）
 if(isEdit&&!isCurrent){
  html+='<button onclick="_modelDialogDelete(\''+escapeHtml(editId)+'\')" style="padding:8px 14px;border-radius:8px;border:1px solid rgba(239,68,68,0.2);background:transparent;color:#ef4444;cursor:pointer;font-size:12px;opacity:0.7;transition:opacity 0.15s;" onmouseenter="this.style.opacity=\'1\'" onmouseleave="this.style.opacity=\'0.7\'">'+t('settings.models.delete')+'</button>';
 }
 ...
 html+='<button onclick="_modelDialogSave(\''+escapeHtml(isEdit?editId:'')+'\')" style="padding:8px 18px;border-radius:8px;border:none;background:#6366f1;color:#fff;cursor:pointer;font-size:13px;">'+t('save')+'</button>';
 html+='</div></div></div>';
