...
1. 先判断当前用户输入里有没有 **情绪波动 / 回忆线索 / 任务连续性** 这类 cue
2. 先从 `L3 经历记忆 + 最近一段 L2 持久记忆` 里做 **联想式候选扫描**
3. 只有在 `repair（修复）/ failure（失败）/ success（成功追问）` 这类强任务连续性场景里，才允许很窄地借一步 `L2 search_relevant` 做 backstop（兜底）
4. 如果命中，生成一段可忽略的联想 hint
5. 注入 prompt，让 LLM 自己决定要不要自然地提到
...
- `flashback` **不是普通记忆检索**
- 它的主体逻辑是 `state resonance（状态共振）/ task continuity（任务连续性）`
- `search_relevant` 在这里只是极窄的后援，不是主逻辑
- 所以它追求的是“像突然想起一段旧经历”，不是“查到一条相关记录就贴上来”
...
- `retention（保留策略）` 已经显式拆成：
  - `keep（永保类）`
  - `compress（压缩类）`
  - `prune（淘汰候选）`
- `search_relevant（记忆检索）` 现在不再是老式 `keyword match（关键词匹配）`
  它已经改成 **多信号相关度**：
  - 文本相关度
  - 任务/情绪/主题 signal profile（信号画像）
  - retention 层级
  - memory_type 类型权重
  - freshness / hit_count
  - 同主题结果去重
...
> **L2 的入口仍然偏宽，但内部已经开始靠 retention + 多信号检索做优选。**
...
- 大模型在开发和修复时，很容易本能地先写一条 `if 关键词命中 -> 走某条路径`
- 这种做法短期看起来快，但很容易把主链重新拉回 `rule-based routing（规则路由）`
- 一旦这样做多了，系统的体感就会从“LLM 主导的自然理解”退化成“命中字面词才会动”

所以现在的边界要明确成：

- `keyword / regex / trigger words` 可以存在
- 但它们最多只能做：
  - `light gate（轻门槛）`
...
- `tool_call / CoD`：主决策仍然交给 LLM，不是关键词先分流
- `L2 search_relevant`：已经改成多信号相关度，不是单纯关键词命中
- `flashback`：主体是 `state resonance（状态共振）/ task continuity（任务连续性）`，不是普通关键词检索
- `dialogue_context`：只做轻量增量提示，不做关键词控制器

如果以后继续改这套系统，看到自己第一反应是“先加几个关键词判断吧”，就应该先停一下，确认是不是又在往旧路退。

---

## 7.4 LLM 主导 vs 电脑操作底座
...
### 最后再补一条：是不是又在往关键词匹配回退？

这是这套系统最容易被大模型反复犯的错误之一。

- 如果一个改动的核心是“多加几个关键词判断”
- 或者“先在最前面插一层规则再说”
- 或者“把 flashback / 记忆 / 规划改成关键字命中后强触发”
...
这里还要补一条现在已经明确下来的设计原则：

- `6 层协议底座` 本身就是技能的底座
- 凡是 `通用型能力`，都应该尽量下沉到底座，而不是为每个动作单独长一层零碎 skill
- 这样它才能变成所有技能的共用能力，也方便后面用户安装新技能时，天然继承同一套目标解析 / 状态校验 / drift / repair / 动作结果语义
- 也就是说，底座要做的是“牵一发动全身”的共用能力，不是围着单一场景打补丁

`write_file（写文件）` 现在就应该按这个口径理解：

- 对外，它仍然以 tool / skill 调用名 `write_file` 暴露给 LLM
- 对内，它不该只是一个零散小技能，而应该是 `6 层协议底座` 里的通用文件动作
- 它现在更接近“底座协议动作的一个对外接口”，而不是“独立业务技能”
- 所以缺参数、写入目标保护、写后校验、verification 语义、重试提示，应该优先收口到底座，而不是继续散落在上层桥接里

### 11.2 最近这一轮补的是“成功后收尾协议”

这次又暴露出一个很关键的问题：

- 有些动作其实已经成功了
- 但模型最后只吐了一句“我来处理 / 马上就好”这类前导话
- 或者把旧式 `<minimax:tool_call>` / `<invoke ...>` 标记混进了最终回复
- 用户侧体感就会像“已经做完了，但最后卡住没回话”

这类问题现在不再往 `routes/chat.py` 主链里补，而是收在：

- `core/reply_formatter.py`

新的边界是：

- `routes/chat.py` 继续只做入口编排，不改主顺序
- `6 层协议底座` 负责把动作结果做成结构化协议
- `reply_formatter` 只负责“把底座结果干净收尾给用户”

现在成功后的收尾规则是：

- 如果模型给出了正常可见的最终回复，就直接用它
- 如果模型最后只剩前导话、承诺话、或空尾巴，就自动改成协议化收尾
- 如果最终文本里混入旧式 tool-call 标记，先剥掉内部标记，再决定是否需要补收尾
- 收尾优先读：
  - `action_summary`
  - `verification_detail`
  - 最后一次真实 `tool_response`

也就是说，这次修的不是“让模型更会说漂亮话”，而是：

> **动作已经完成时，系统必须保证最后一跳能落成一条干净、可见、可交付的结果回复。**

### 11.3 `verification_detail` 已经开始往 L6 / 前端透传
