...
- `CLAUDE.md` 现在更适合作为**项目手册 / 约束汇总 / 快速入口**，不再作为当前运行态的唯一权威来源。
- 当前运行态主链请优先看 `docs/10-架构-architecture/AaronCore_当前真实流程文档.md`。
- L1-L8 分层语义请看 `docs/10-架构-architecture/8层Brain架构详解.md`。
- 文档冲突时的优先级：**代码 > 当前真实流程文档 > 8 层分层文档 > CLAUDE.md**。
...
- **L5 = 成功方法经验**（什么做法验证过）→ 主要来自验证成功的 L6 run 上浮
- **L6 = 执行轨迹**（这次到底怎么跑的）→ 记录 success / verified / drift / verification facts
- **L7 = 纠偏约束**（别再怎么做）→ 存用户负反馈和失败提醒，不负责正向方法沉淀
- **L8 = 已学知识**（我学了什么）→ 只保留可复用知识卡片，按需检索
- L3 **禁止存**用户事实（→ L4）、交互规则（→ L4）、知识内容（→ L8）
- L4 **只读** persona.json，**不再交叉读** long_term.json
- 不要把 L7 的 `fix` 当成新的 L5 方法库；正向经验归 L5，负向约束归 L7
- L8 的 `feedback_relearn` 类型条目不参与正式知识检索（`should_surface_knowledge_entry` 过滤）
...
#### L2 持久记忆中枢 — `memory/l2_memory.py` + `l2_short_term.json`（中枢分发）

- 文件：`state_data/l2_short_term.json`（记忆存储，不设上限）+ `state_data/l2_config.json`（轮次统计）
- 代码：`memory/l2_memory.py`（评分入库 + 关键词检索 + 自动结晶 + 每20轮摘要）
...
#### L5 成功方法经验 — `knowledge.json`

- 加载：`storage/state_loader.load_l5_knowledge()`
- 当前返回内容分两块：
  - 最近 10 条 `knowledge.json` 条目
  - 当前已注册技能的轻量元数据（`name` + `keywords`）
- 当前 L5 的核心来源不是“技能需求信号”，而是 `memory.evolve() -> _maybe_promote_success_path()` 把 **verified + success + 无 drift** 的 L6 run 上浮成 `source="l6_success_path"` 的成功经验
- 作用：沉淀“什么做法被验证有效”的正向经验，供复杂任务和按需上下文复用
- 典型成功经验字段：
  - `experience_key` / `name`
  - `action_kind` / `target_kind`
  - `outcome` / `observed_state`
  - `verification_mode` / `verification_detail`
...
#### L6 执行轨迹 — `evolution.json`

- 加载：`memory.get_evolution()`
- 写入：`memory.evolve(user_input, skill_used, run_event)`
- 作用：保存真实执行事实，而不是直接提供给 prompt 的“知识”
- 当前主要有两块：
  - `skills_used`：每个技能的 `count` / `verified_count` / `failure_count` / `drift_count` / `last_outcome`
  - `skill_runs`：逐次执行事实，记录 `success` / `verified` / `expected_state` / `observed_state` / `drift_reason` / `repair_hint` / `repair_succeeded` / `verification_mode` / `verification_detail`
- L6 是：
  - L5 成功经验上浮的原料层
  - 前端 `execution_trace` 时间线的事实来源
  - 后续纠偏与验证分析的素材层

#### L7 反馈纠偏 — `feedback_rules.json`

- 记录入口：`feedback/classifier.py::record_feedback_rule()`
- 主链写回：`routes/chat.py` 回复后调用 `S.l7_record_feedback_v2(...)`
- 作用：保存用户负反馈、失败提醒和纠偏约束，主链只注入少量 relevant rules 给模型参考
- 当前 L7 更接近：
  - failure spec
...
- 当前 L7 **不是**：
  - 正向方法库（那是 L5）
  - 修复任务状态机（那应该落到 task / repair runtime）
- 规则通常包含：`category` / `scene` / `problem` / `fix` / `last_question` / `user_feedback` / `feedback_count`
- `feedback_relearn` 仍可能作为补学预览存在，但不再作为 L8 正式知识卡片参与检索
- 旧的 `_extract_routing_constraint()` 已退役，不要再把 L7 当成新的路由表

#### L8 已学知识 — `knowledge_base.json` + `autolearn_config.json`

- 主实现：`memory/l8_learning.py`（`core/l8_learn.py` 只是兼容 shim）
- 搜索：`query_knowledge` / `find_relevant_knowledge()` 按需检索，不全量注入
- 写入：`save_learned_knowledge()`
- 当前正式入库来源：
  - 自主 / 显式学习：`auto_learn()` / `explicit_search_and_learn()` 触发 web 搜索后入库
  - 对话结晶：`memory/l2_memory.py::_to_l8()` 把知识型对话沉淀为 `source="l2_crystallize"` 的知识卡片
- 作用：保存真正可复用的事实 / 概念 / 原理 / 方法性知识，供后续按需召回
- `feedback_relearn` 只保留在反馈链路中，并被 `should_surface_knowledge_entry()` 过滤，不参与正式知识检索或时间线展示
- 入库前会经过：
  - 摘要凝结
  - entry quality 检查
  - query 规范化
