...
NovaCore is a desktop AI assistant ("Nova") with a FastAPI backend, single-page HTML frontend, and a multi-layer memory system (L1-L8). It uses a user-configurable LLM as its brain (via `/models/config` 管理), with rule-based routing taking priority over LLM routing.
