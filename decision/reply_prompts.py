...
def build_light_chat_prompt(bundle: dict, *, build_recent_dialogue_text) -> str:
    ...
    light_rules = cfg.get(
        ...
        cfg.get(
            "reply_rules",
            [
                "\u8fd9\u662f\u666e\u901a\u804a\u5929\uff0c\u76f4\u63a5\u81ea\u7136\u63a5\u8bdd\uff0c\u4e0d\u8981\u5148\u505a\u957f\u7bc7\u94fa\u57ab\u3002",
                "\u76f4\u63a5\u57fa\u4e8e\u4e0a\u9762\u7684 L1/L2/L4/L7 \u56de\u7b54\uff0c\u4e0d\u8981\u8bf4\u81ea\u5df1\u6ca1\u6709\u8bb0\u5fc6\u3002",
                "\u7528\u6237\u8ffd\u95ee\u65f6\u9ed8\u8ba4\u6cbf\u7740\u6700\u8fd1\u8bdd\u9898\u63a5\u4e0a\uff0c\u4e0d\u8981\u53cd\u95ee\u201c\u4f60\u6307\u4ec0\u4e48\u201d\u3002",
...
def build_cod_system_prompt(bundle: dict, *, build_fs_focus_guidance) -> str:
    ...
    rules_list = cfg.get(
        "reply_rules",
        [
            "你拥有顶级的记忆系统，禁止说“我没有记忆”“我记不住”。需要回忆时调用 recall_memory。",
...
def build_tool_call_system_prompt(bundle: dict, *, build_fs_focus_guidance) -> str:
    ...
    tool_guidance = [
        ...
        "Only auto-continue or resume a previous task when the current user prompt includes the explicit task continuity block. If that block is absent, treat earlier dialogue as background context only and ask a short clarification instead of executing tools.",
        "If the current user prompt includes 'Turn execution policy: explain_only', answer from the runtime state and end the turn. Do not call tools, inspect the environment, or continue execution in that same turn.",
