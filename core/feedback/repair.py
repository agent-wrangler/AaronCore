...
def _route_candidate_files(text: str) -> list[dict]:
    ...
    if _contains_any(text, ("代码", "编程", "游戏", "run_code")):
        candidates.append({"path": "tools/agent/run_code.json", "reason": "如果编程技能关键词太宽，会误吸“能力提问/元提问”。"})
    if _contains_any(text, ("天气", "温度", "多少度", "乌鲁木齐", "新疆")):
        candidates.append({"path": "skills/builtin/weather.py", "reason": "天气技能的地点提取和追问继承都在这里。"})
    if _contains_any(text, ("故事", "太短", "讲长一点", "继续讲")):
        candidates.append({"path": "skills/builtin/story.py", "reason": "故事长度和续写内容在这里生成。"})
...
def _story_length_files() -> list[dict]:
    return _dedupe_items(
        [
            {"path": "skills/builtin/story.py", "reason": "故事正文长度、章节衔接和持久化状态都在这里。"},
