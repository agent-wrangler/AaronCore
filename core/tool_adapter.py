...
def _is_generic_shell_directory(path: str) -> bool:
    ...
    if not raw:
        return False
    try:
        tail = _Path(raw.replace("/", "\\")).name.strip().lower()
    except Exception:
        return False
...
def _apply_protocol_context(name: str, ctx: dict, user_input: str, tool_args: dict | None = None) -> dict:
    ...
    if fs_target and _is_generic_shell_directory(str(fs_target.get("path") or "")) and not allow_generic_reuse:
        ...
        if _is_generic_shell_directory(str(ctx.get("path") or "")):
            ctx["path"] = ""
    if not fs_target:
        inherited_context_target = _normalize_protocol_fs_target(context_data.get("fs_target"), default_option=default_option)
        if inherited_context_target and _is_generic_shell_directory(str(inherited_context_target.get("path") or "")) and not allow_generic_reuse:
    ...
    try:
        ...
        pulled = pull_context_data(user_input or current_candidate_path, _PROTOCOL_CONTEXT_MANIFESTS.get(name, {}))
    except Exception:
        pulled = {}
    ...
    if not fs_target:
        ...
        if task_plan:
            ...
            if candidate_target and _is_generic_shell_directory(str(candidate_target.get("path") or "")) and not allow_generic_reuse:
